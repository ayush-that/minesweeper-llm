...
for prompt_name, prompt_cfg in PROMPTS.items():
    ...
    for rows, cols, mines, n_games in board_configs:
        for seed in range(n_games):
            ...
            for move_i in range(max_moves):
                ...
                prompt = build_frontier_prompt(board, rows, cols, mines, flags, extra_rules)
    ...
    flag_acc = total_correct_flags / max(total_correct_flags + total_wrong_flags, 1) * 100
    ...
    print(f"[{prompt_name}] AvgScore={avg_score:+.1f} | Wins={total_wins}/{total_games} | "
...
# === LEADERBOARD ===
print(f"\n{'='*70}")
print(f"  LEADERBOARD (sorted by avg score)")
print(f"{'='*70}")
ranked = sorted(results.items(), key=lambda x: x[1]["avg_score"], reverse=True)
for i, (name, r) in enumerate(ranked):
    medal = ["🥇", "🥈", "🥉", "4.", "5.", "6."][i]
    print(f"  {medal} {name:20s} | AvgScore={r['avg_score']:+6.1f} | MineHit={r['mine_rate']:.0f}% | FlagAcc={r['flag_acc']:.0f}% | Wins={r['wins']}")
